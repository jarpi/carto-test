...
const validateParams = (z, x, y_coord_and_format, style) => {
    const matchedGroups = y_coord_and_format.match(COORD_Y_AND_FORMAT_RE);
    if (!matchedGroups) return Promise.reject({statusCode: 400,msg:'Invalid Y coord'});
    if (!matchedGroups[2]) return Promise.reject({statusCode:400, msg:'Invalid Y coord'});
    if (!matchedGroups[3]) return Promise.reject({statusCode:400, msg:'Invalid format'});
    const y = matchedGroups[2];
    const format = matchedGroups[3];
    const maxTilesPerZoomLevel = (((z*2)-1) > 0 ? (z*2)-1 : 0);
    const paramsAreValid =
        (z >= MIN_ZOOM_LEVEL && z <= MAX_ZOOM_LEVEL)
        && (x >= 0 && x <= maxTilesPerZoomLevel)
        && (y >= 0 && y <= maxTilesPerZoomLevel)
...
const loadMap = style => {
    ...
    return new Promise((resolve, reject) => {
        // check path exists first, error if not
