...
const validateParams = (z, x, y_coord_and_format, style) => {
    ...
    if (!matchedGroups || !matchedGroups[2]) return Promise.reject({statusCode:400, msg:'Invalid Y coord'});
    if (!matchedGroups || !matchedGroups[3]) return Promise.reject({statusCode:400, msg:'Invalid format'});
